    ...
    try {
        ...
        if (document) {
            ...
            if (textDoc) {
                ...
                console.log("no text/ doc FOUND");
                res.send({ status: 404, data: `NO text/ doc FOUND` })
            }
        }
    } catch (error) {
        console.log(error);
...
app.post("/text-upload", async (req, res) => {
    ...
    // console.log(textCode, userText);
    try {
        await textModel.create({ textCode, userText })
        console.log("text and code uploaded");
        res.send("Text Uploaded to the data base")
        setTimeout(async () => {

            //deleting from db
            await textModel.deleteMany({ textCode })
            console.log(`Deleted CODE: ${textCode} from DB`);

        }, 240000);
    }
    catch (e) {
        console.log("ERROR AT UPLOADING TEXT TO DB", e);
